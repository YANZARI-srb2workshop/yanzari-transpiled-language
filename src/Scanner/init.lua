...
Scanner.rules = {
	...
	--
	-- Spoiler:
	-- - Maybe we'll have an extension for HTML tags.
}

-- An array
---@generic tableValue: any the table value.
---@alias array<tableValue> {[number]: tableValue} 

-- Creates a ipairs loop without one of the variables.
---@generic tableValue: any the table value.
---@param tbl array<tableValue> a table for the ipairs iterator to run the loop
---@param callback fun(tableValue: tableValue):boolean a callback for the ipairs iterator while it runs the loop (If it returns false, it causes the loop to terminate)
local function createAIpairsLoopWithoutOneVariable(tbl,callback)
	for _,tableValue in ipairs(tbl) do
		local callbackValue = callback(tableValue)
		assert(type(callbackValue)=='boolean','The value returned by callback is not a boolean.')
		if callbackValue==false then
			break
		end
	end
end
...
---@generic functionReturnedValues: ...
---@generic functionParameterValues: ...
---@param func fun(functionParameters: functionParameterValues): functionReturnedValues a function that turns into a thread
---@return fun(functionParameters: functionParameterValues): functionReturnedValues func a function that resumes the coroutine each time it is called.
local function coroutineWrap(func)
	return coroutine.wrap(func)
end

-- Run Scanner Rules
---@async
---@param this Scanner self
---@return Token|Token[]? token_or_output the tokens generated by the Scanner.
function Scanner.runRules(this)
	local threadWrapper = coroutineWrap(
	---@param self Scanner self
	---@return Token|Token[]? token_or_output the tokens generated by the Scanner.
	function(self)
		while true do
			createAIpairsLoopWithoutOneVariable(Scanner.rules,function(rule)
				if self.current==nil then
					return false
				end
				...
				createAIpairsLoopWithoutOneVariable(token.token,function(char)
